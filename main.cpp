...
int main(int argc, char* argv[])
{
    if (argc == 3)
    {
        ...
        std::cout << "DFS recursive approach\n";
        ...
        std::cout << "DFS iterative approach\n";
        ...
        std::cout << "BFS iterative approach\n";
        pathExists = graph->pathExistsBFSIter(originNode, destNode, visited);
        if (pathExists)
        {
            std::cout << "Path from node " << originNode << " to node " << destNode << "\n";
            for (auto node : visited)
